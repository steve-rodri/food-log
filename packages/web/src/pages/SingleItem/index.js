...
const CommonFoods = ({ data, onSelect }) => {
  ...
  const range =
    data.length > 5 ? [...Array(5).keys()] : [...Array(data.length).keys()];
  ...
  return (
    <Fragment>
      ...
      <div className="SingleItem-Results-Group">
        {range.map(i => (
          <Food key={i} food={data[i]} onSelect={() => onSelect(data[i])} />
        ))}
      </div>
    </Fragment>
  );
...
const LogButton = () => {
  ...
  const style = () =>
    basket.contents && basket.contents.length
      ? buttonStyles.greenhighlight
      : null;
