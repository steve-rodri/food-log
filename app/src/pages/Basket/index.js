...
import { deleteBasketItem, resetBasket, addBasketToLog, setView, handleSelect, clearResults } from '../../redux'
import './style.css';
...
const EmptyButton = () => {
  const dispatch = useDispatch()
  return (
    <button className="Basket-Empty delete-button" onClick={() => dispatch(resetBasket())}>
      Empty
    </button>
  )
}
...
const BackButton = () => {
  const dispatch = useDispatch()
  return(
    <button onClick={() => dispatch(setView('SingleItem'))}>
      Back
    </button>
  )
}
...
const LogButton = ({ onSubmit }) => {
  ...
  return (
    <button
      className={buttonStyles.greenhighlight}
      onClick={onSubmit}
    >
