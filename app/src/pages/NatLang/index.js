...
import { handleNatLangQuery, setView } from '../../redux'
import { getMealTypebyTime, timeOfDay } from '../../helpers'
import './style.css';
...
const NatLang = () => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    if (query.length) handleNatLangQuery(query);
...
const Greeting = () => {
  ...
  const { data: { givenName } } = user
  return (
    <div className="NatLang-Greeting" >
      {givenName? <h2>{`Good ${timeOfDay()} ${givenName},`}</h2> : null}
      <h2>What's for {getMealTypebyTime()}?</h2>
    </div>
  )
}
